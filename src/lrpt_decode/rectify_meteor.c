    ...
    *( rect_buff + rect_buff_left-- ) = byteA_left;
    ...
    for( idx = 0; idx < in_width2 - 1; idx++ )
    {
      ...
      switch ( *(gap + idx) )
      {
        ...
        default:
          fprintf(stderr, _("\nmlrpt: A problem occurred spacing the pixels\n") );
          break;
      } /* switch ( *( gap + idx) ) */
...
Rectify_Images( void )
{
  ...
  {
    case 1:
      Print_Message( _("Using Rectify Function 1 (W2RG)"), INFO_MESG );
      Calculate_Pixel_Spacing_1( METEOR_IMAGE_WIDTH, &channel_image_width );
      break;

    case 2:
      Print_Message( _("Using Rectify Function 2 (5B4AZ)"), INFO_MESG );

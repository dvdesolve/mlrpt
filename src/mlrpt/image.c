...
void Normalize_Image(
  ...
  {
    Print_Message(
        "Image buffer empty? Normalization not performed", ERROR_MESG );
    return;
  }
  ...
  {
    Print_Message(
        "Image buffer flat? Normalization not performed", ERROR_MESG );
    return;
  }

  /* Perform histogram normalization on images */
  Print_Message( "Performing histogram normalization", INFO_MESG );
...
void Flip_Image(uint8_t *image_buffer, uint32_t image_size) {
  ...
  {
    Print_Message(
        "Image buffer empty? Rotation not performed", ERROR_MESG );
    return;
  }

  /* Rotate image 180 degrees */
  Print_Message( "Rotating image by 180 degrees", INFO_MESG );
